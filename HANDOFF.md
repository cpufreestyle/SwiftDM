...
15. **桌面端筛选栏补上键盘导航提示**（`13ccd1c`）：↑↓/Return 的快捷键在桌面端早就接好了
    （`_setup_shortcuts`），但界面上一个字都没提；Web 端筛选栏里一直挂着一行 `.kbd-hint`
    「↑↓ 选择任务 · Enter 打开」。同一个能力在一边可发现、在另一边完全隐形，这是三端对齐的最后一处缺口。
   - `main_window.py`：`QLabel#kbdHint`（11px + `$textMuted`，与 Web 端 `--text2` 同值，不新增令牌），
     放在四颗筛选芯片之后、`addStretch` 之前，跟着芯片在左边而不是漂到排序/搜索那一侧；
   - 同时给 `setToolTip`（悬浮说明）与 `setAccessibleName`（读屏可读）；
   - 测试：`tests/test_desktop_ui.py` 新增 `test_filter_bar_hints_at_the_task_keyboard_shortcuts`，
     断言文案、可访问名、tooltip、QSS 规则、Web 端提示仍在、离屏渲染后尺寸非零，
     并用「伸缩位」作为左右分组边界来钉住位置；变异测试 5/5 全红。
   - 踩坑记录：① 位置断言最初只写了「在 sort_combo 之前」，把提示挪到 `addStretch` 之后依然能过，
