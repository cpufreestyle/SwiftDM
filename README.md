...
- 🔗 **便捷操作**：任务卡片支持**复制下载链接**、**打开所在文件夹**、一键打开下载目录**、紧凑模式一键切换（隐藏次要信息，选择会被记住）**、**任务详情面板**（桌面端双击卡片 / Web 端「详情」按钮：分段进度、失败原因与建议实时刷新，可一键复制详情，任务删除后面板自动关闭）、批量复制可见任务链接 / 导出任务列表 CSV**；桌面端快捷键 `Ctrl+N` 新建下载、`Ctrl+F` 定位链接输入框、`↑`/`↓` 在任务间移动选中、`回车` 打开选中任务的文件
