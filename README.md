...
- 🧲 **BT / PT 下载**：支持磁力链接（`magnet:`）与 `.torrent` 种子文件，含私有 Tracker（PT）下载；实时显示做种数🌱 / peer 数👥，下载完成后自动做种以满足分享率
- ⏸ **断点续传**：随时暂停 / 继续，恢复时自动从断点续下；分段下载中途断流自动重试
- ✅ **完整性校验**：合并前校验每个分段与最终文件大小，杜绝静默截断
- 🌐 **浏览器监控**：Chrome 扩展自动捕获浏览器中的下载
- 📋 **剪贴板监听**：复制下载链接自动检测并提示（托盘气泡点击才添加，完成/失败等提示点击无副作用）；浏览器扩展捕获下载时同样即时提示来源
- 🖥 **原生桌面 UI**：基于 PyQt6 的暗色主题界面，IDM 风格任务卡片；支持**拖拽链接/磁力即下载**、**状态分段过滤**（全部 / 进行中 / 已完成 / 失败）与任务**右键菜单**、任务**关键字搜索**、列表排序**
- 📊 **实时进度**：速度、ETA 倒计时、进度百分比实时显示；多线程任务的**分段进度**逐段展示（桌面端详情面板 / Web 端卡片进度条下方，段满变绿，悬停查看每段字节）
- 🔔 **完成通知**：系统托盘气泡 + 状态栏通知；托盘图标随状态变化（下载中 / 空闲 / 有失败任务）；完成提示音可设置（关闭 / 提示音 / 系统音）
- ⏰ **完成后动作**：设置「全部下载完成后」关机 / 睡眠 / 提示音；任务清空后进入 60 秒倒计时，桌面端工具栏与 Web 端均显示剩余时间并可一键取消；设置会记住，重启后仍生效
- 🗂 **批量管理**：全部暂停 / 恢复、重试失败任务、清除已完成任务（有确认）
- 💾 **状态记忆**：过滤分段、窗口大小/位置在重启后自动恢复
- 🔗 **便捷操作**：任务卡片支持**复制下载链接**、**打开所在文件夹**、一键打开下载目录**、紧凑模式一键切换（隐藏次要信息，选择会被记住）**、**任务详情面板**（桌面端双击卡片 / Web 端「详情」按钮：分段进度、失败原因与建议实时刷新，可一键复制详情，任务删除后面板自动关闭）、批量复制可见任务链接 / 导出任务列表 CSV**；桌面端快捷键 `Ctrl+N` 新建下载、`Ctrl+F` 定位链接输入框、`↑`/`↓` 在任务间移动选中、`回车` 打开选中任务的文件；Web 端同样支持 `Ctrl+N` 聚焦链接框（并全选）、`Ctrl+F` 聚焦搜索框、`Esc` 关闭详情/设置弹窗
- 🌐 **桌面 / Web 双端一致**：状态分段过滤、拖入即下载（链接 / 磁力 / 本地 `.torrent` 种子文件）、总体进度在 PyQt6 桌面与纯 Web 界面下均可使用
- 🌙 **主题**：Web 端支持浅色 / 深色 / 跟随系统（默认跟随系统），选择会被记住；桌面端支持深色 / 浅色两套主题（设置 → 外观 → 界面主题，改完即时预览，选择会被记住）
...
SwiftDM 内置 BT 客户端（基于 libtorrent），可直接下载磁力链接与种子文件，无需额外工具：

- **磁力链接**：在输入框粘贴 `magnet:?xt=urn:btih:...` 即可；
  私有 Tracker（PT）的磁力链接通常带有 `&tr=https://...` Tracker 参数，已自动支持。
- **种子文件**：粘贴以 `.torrent` 结尾的下载地址（如 PT 站点的种子下载链接），
  SwiftDM 会先拉取 `.torrent`，其中的 announce / passkey 已内嵌，可直接做种源交换。
- **PT 合规**：私有种子（torrent 标记为 private）会自动禁用 DHT / PEX / LSD，
  仅通过种子/磁力携带的 Tracker 交换做种源，符合 PT 站点规则；下载完成后继续做种以满足分享率。
- 任务卡片实时显示 🌱 做种数 / 👥 peer 数；支持暂停 / 继续 / 取消 / 重试，与 HTTP 任务一致。

> 说明：对等连接（peer）一律直连，不走 HTTP 代理；仅当设置环境变量
> `SWIFTDM_TORRENT_PROXY=http://127.0.0.1:7897` 时，Tracker 的 HTTP announce 才走该代理
> （peer 仍直连）。多数家庭网络下 Tracker 可直连，无需设置。
...
- 弹出面板头部实时显示下载中数量 / 总速度 / 失败数（打开即刷新，每 2 秒轮询）
